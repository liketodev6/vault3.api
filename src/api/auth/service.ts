...
class AuthServices {
  ...
  public validatePayment = async (user: User, body: ValidatePaymentReqModel): Promise<IResponseModel<string>> => {
    ...
    if (!pay.success) return getResponse(false, 'Payment error', null);
    const storage = await this.StorageManager.create(user, body);
    await this.TransactionManager.create({ //! add transactionId to storage table
      user: user.id,
      storage: storage,
    ...
    await this.FileManager.createStorageRootDirectory(StorageHelper.generateStorageDirectory(user.id, storage));
